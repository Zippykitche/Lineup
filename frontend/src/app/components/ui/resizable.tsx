    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          const key = String(nameKey || item.name || item.dataKey || `value-${index}`);
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
...
function ChartLegendContent({
  ...
  return (
    ...
    >
      {payload.map((item, index) => {
        const payloadKey = item.payload?.dataKey ?? item.payload?.name ?? item.value ?? `item-${index}`;
        const key = String(nameKey || payloadKey || "value");
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
